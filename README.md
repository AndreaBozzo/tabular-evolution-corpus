...
| `add_required_column` v0→v1 | (absent) *v0* | `VARCHAR` *v1* | error: `SchemaError` | `String` *v1* | (absent) *v0* | `string` *v1* |
...
| `int32_to_decimal` v0→v1 | `INTEGER` *v0* | `DECIMAL(12,2)` *v1* | error: `SchemaError` | `Decimal(precision=12, scale=2)` *v1* | `int32` *v0* | `decimal128(12, 2)` *v1* |
...
| `timestamp_ns_to_us` v0→v1 | `TIMESTAMP_NS` *v0* | `TIMESTAMP_NS` *v0* | error: `SchemaError` | `Datetime(time_unit='us', time_zone=None)` *v1* | `timestamp[ns]` *v0* | `timestamp[ns]` *v0* |
| `widen_int32_to_int64` v0→v1 | `INTEGER` *v0* | `BIGINT` *v1* | error: `SchemaError` | `Int64` *v1* | `int32` *v0* | `int64` *v1* |

Every adapter and mode reads the other 10 alike: `add_all_null_column` v1→v2, `column_becomes_all_null` v0→v1, `dictionary_reencoded` v0→v1, `empty_then_populated` v0→v1, `nullable_column_backfilled` v0→v1, `nullable_to_required` v0→v1, `reorder_columns` v0→v1, `required_to_nullable` v0→v1, `rows_appended_without_key` v0→v1, `rows_deleted` v0→v1.

**Reading each version on its own** (`from` → `to`). Adapters disagree on 7 of 36 transitions; here the differences are in how each engine represents a single file.
